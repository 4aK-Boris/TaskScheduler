...
// Browser entry point. Composition root: hand-wires repositories and UseCases, then hands
// them to DefaultRootComponent, which the React tree reads through `useValue`. No DI container
// in the browser bundle — it would only grow the JS payload and hide the wiring.
...
fun main() {
    ...
    // Paint the persisted palette onto <html> BEFORE the first React render, otherwise a
    // dark-mode user gets one frame of the light theme.
    applyThemeMode(root.isDarkTheme.value)

    val container = document.getElementById(ElementId("root"))
        ?: error("index.html is missing the #root mount point")
