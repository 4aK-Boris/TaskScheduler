...
**Все события всем подключённым клиентам**, клиент фильтрует. Subscribe-with-query — Phase 2.
...
Phase 2 — можно добавить вторую кнопку "Retry +1" если будет реальный кейс.
...
- **/jobs/{id}** — JobDetail (state timeline из `job_event`, payload viewer, retry/cancel/delete actions, dependencies)
...
- **DependencyGraph** (Phase 2) — визуализация DAG для конкретного job-а
...
2. **Reference impl: `FileArchivalSink(baseDir: Path)`** — пишет JSONL под `baseDir/<category>/<YYYY-MM-DD>.jsonl`. Append-mode + UTC partitioning, не fsync — для dev/single-node ОК, для compliance writer-у нужен S3/GCS wrapper.
3. **Cleanup pipeline** (`RetentionCleanupBatchUseCase`): `SELECT terminal WHERE updated_at < cutoff` → `ArchivalSink.archive(...)` → `DELETE WHERE id IN (...) AND state = ?`. Если sink бросает — DELETE для этого batch ПРОПУСКАЕТСЯ (log.error), rows доживают до следующего тика, sink получает второй шанс. Другие buckets в этом же тике не страдают.
4. **Архивируется только `job` row.** `job_event`, `job_dependency`, `outbox` каскадно удаляются с parent — если пользователю нужен полный audit lineage, он подключает свой собственный sink и читает их сам перед DELETE.
