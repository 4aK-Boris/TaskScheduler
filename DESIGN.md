...
Раз в 30с на каждом ноде — две независимые потери, два разных пути спасения:

- **Умер воркер посреди работы** (`RecoverOrphanedJobsUseCase`): `PROCESSING` с истёкшим
  `locked_until` → `AWAITING_RETRY` + строка в outbox.
- **Потерялось сообщение брокера** (`RecoverStuckEnqueuedJobsUseCase`): строка висит в
  `ENQUEUED`, сообщения за ней нет и не будет. Первый путь её не покрывает: у `ENQUEUED` нет
  лока, которому истекать.

🔴 Одного возраста для второго случая недостаточно: джоба, честно ждущая своей очереди за
длинным предшественником (`q.heavy`, concurrency 1), выглядит точно так же. Различает их
**обгон** — брокер отдаёт очередь в порядке (приоритет, публикация), поэтому если в той же
очереди уже стартовали более **поздние** джобы приоритета не выше, сообщения за нашей строкой
быть не может. Восстанавливаем только когда выполнены оба условия: строка не менялась дольше
`stuckEnqueuedThreshold` (по умолчанию 5 минут) **и** обогнана.

Перепубликация безопасна, потому что pickup — это CAS по `(id, version)`: если оригинальное
сообщение всё же придёт, одна доставка выиграет, вторая будет подтверждена без выполнения.
Состояние строки не меняется, двигается только `updated_at` — чтобы следующий скан не
перепубликовал ту же джобу раньше следующего окна.

Зачем это вообще: одна потеря сообщения останавливает целый recurring-канал бессрочно —
`SKIP`-overlap не ставит следующий тик, пока предыдущая джоба не терминальна. Прод-инцидент
2026-09-09: 18 джоб `q.default`, опубликованных во время рестарта воркера, не дошли, и приём
трейд-офферов, продажа и доставка стояли 74 минуты до ручного вмешательства.
...
- **2026-09-09** — **Потерянное сообщение больше не глушит канал навсегда, а отбраковка перестала быть невидимой** (0.8.8). Два независимых дефекта надёжности, вскрытых одним прод-инцидентом.

  **(1) Джоба, чьё сообщение не дошло до брокера, висела в `ENQUEUED` вечно.** Orphan-recovery реклеймит только `PROCESSING` по истёкшему `locked_until` — у `ENQUEUED` лока нет, реклеймить нечего. Сама по себе одна потеря переживаема; смертельным её делает `SKIP`-overlap, который не ставит следующий тик, пока предыдущая джоба не терминальна: канал встаёт бессрочно. 2026-09-09 в CsTradeService так остановились приём трейд-офферов, продажа и доставка — 18 джоб `q.default`, опубликованных в окно рестарта воркера (`outbox.published_at` заполнен, в очереди пусто, в `job_event` ни строки), простояли 74 минуты до ручной отмены. Тот же канал уже вставал на 61 минуту сутками раньше. Показательно, что 7.3 описывал этот второй путь спасения с самого начала — реализована была только вторая его строка, orphan recovery.

  `RecoverStuckEnqueuedJobsUseCase` в `SafetyNetPoller` закрывает дыру. 🔴 Возраста для детекта недостаточно: джоба, честно ждущая очереди за длинным предшественником (`q.heavy`, concurrency 1), выглядит точно так же и наивным критерием перепубликовывалась бы все три часа обучения. Различает их **обгон** — брокер отдаёт очередь в порядке (приоритет, публикация), поэтому если в ней уже стартовали более поздние джобы приоритета не выше, сообщения за нашей строкой быть не может. Требуются оба условия: не менялась дольше `stuckEnqueuedThreshold` (5 минут, `null` выключает) **и** обогнана. Перепубликация безопасна, потому что pickup — CAS по `(id, version)`: пришедший позже оригинал будет подтверждён, ничего не выполнив. Состояние строки не меняется, двигается только `updated_at` — он же держит следующий скан на окно вперёд.

  **(2) Dead-letter терял сообщения молча.** Dead-lettered сообщение уходит в DLX под тем routing key, с которым пришло в очередь (`default`, `marketplace`, …), если у очереди не задан `x-dead-letter-routing-key`. `jobs.dlx` — direct, совпадение точное, а `q.dead-letter` был привязан только под **пустым** ключом: каждое отбракованное сообщение оказывалось unroutable и отбрасывалось без следа. На брокере, прожившем месяцы, DLQ стоял вечно пустым — то есть отбраковка была не потеряна, а невидима. Теперь `RabbitTopology` привязывает DLQ под ключом каждой очереди; binds идемпотентны, поэтому брокер, объявленный старой версией, чинится сам при следующем старте. `x-dead-letter-routing-key` на очередях и fanout вместо direct рассмотрены и отвергнуты: оба меняют уже объявленные сущности и роняют старт существующих инсталляций (`PRECONDITION_FAILED`).
