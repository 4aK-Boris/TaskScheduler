...
текущая версия `0.8.8` — фиксированный релиз без `-SNAPSHOT` (новая версия выходит с бампом
...
(не SNAPSHOT): после правок в библиотеке либо перезалей тот же `0.8.8` (mavenLocal перезапишет),
...
```kotlin
dependencies {
    implementation("cs.trade.scheduler:core-backend:0.8.8")
    implementation("cs.trade.scheduler:storage-postgres:0.8.8")
    implementation("cs.trade.scheduler:transport-rabbit:0.8.8")
    implementation("cs.trade.scheduler:engine-worker:0.8.8")
...
> добавляет этот maven-репозиторий и тянет `cs.trade.scheduler:*:0.8.8`; в CI того же
